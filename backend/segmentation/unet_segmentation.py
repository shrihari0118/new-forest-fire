...
def segment_preprocessed_data(region: str, n_clusters: int = 3, data_root: Optional[str] = None) -> Dict[str, object]:
    """
    Segment a region using KMeans on up to 3 bands of each reference GeoTIFF:
    - For each *_metadata.json in preprocessed_data, segment and save as <basename>_mask.npy
    """
    try:
        ...
        for meta_file in meta_files:
            try:
                ...
                # Save as <basename>_mask.npy (no region prefix)
                base_name = os.path.basename(meta_file).replace("_metadata.json", "")
                mask_filename = f"{base_name}_mask.npy"
                mask_path = os.path.join(segmented_dir, mask_filename)
                ...
                results.append({
                    "ok": False,
                    "region_slug": region_slug,
                    "message": f"Segmentation failed for {meta_file}: {e}"
                })
